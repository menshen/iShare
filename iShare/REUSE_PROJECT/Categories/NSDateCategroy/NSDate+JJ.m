...
+(NSString*)getRealTime:(NSString *)timestamp
    ...
    if (timestamp.length>9) {
        timestamp=[timestamp substringToIndex:9];
    }
...
+(NSString*)getTimeStampLong:(NSDate*)date{
    ...
    NSTimeInterval time = ([timeDate timeIntervalSince1970]); // returned as a double
    long digits = (long)time; // this is the first 10 digits
    int decimalDigits = (int)(fmod(time, 1) * 1000); // this will get the 3 missing digits
     return  [NSMutableString stringWithFormat:@"%ld%d",digits,decimalDigits];
