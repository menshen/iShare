...
//        [self addSubview:_arrowImage = arrowImage];
        ...
        _arrowImage = [[UIImageView alloc] initWithFrame:self.bounds];
        _arrowImage.size=CGSizeMake(35, 35);
        _arrowImage.contentMode = UIViewContentModeScaleAspectFit;
        _arrowImage.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
...
- (void)layoutSubviews
{
    ...
    // 1.箭头
   CGFloat arrowX = self.width * 0.5 - 120;
...
//            
//            else{
//                self.scrollView.contentInset = UIEdgeInsetsMake( MIN( -self.scrollView.contentOffset.y, 60 ), 0, 0, 0 );
//
//            }
            ...
            //显示图片
            for (int i=0; i<8; i++) {
                //,macOSX-%d,box_colored_notify_1@2x，CAM_Processing_Gold_20x20_0001
                NSString * name=[NSString stringWithFormat:@"tt_%d",i+1];
                [array addObject:[UIImage imageNamed:name]];
                
            }
            self.arrowImage.animationImages = array;
            self.arrowImage.animationDuration = (CGFloat)array.count/8;
