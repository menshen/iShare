...
- (void)setupDatasource{
    ...
    NSArray * a_m = [IS_SenceTemplatePanModel objectArrayWithKeyValuesArray:arrayM];
    ...
    if (btn.tag==0) {
        ...
        if (self.actonSheetBlock) {
            if (_curIndexPath.row>=0) {
                IS_SenceTemplatePanModel * p = self.c_datasource[_curIndexPath.row];
...
//    NSDictionary * dic =
    cell.senceTemplatePanModel = self.c_datasource [indexPath.row];//[[IS_SenceTemplatePanModel alloc]initWithDictionary:dic];
    return cell;
}
